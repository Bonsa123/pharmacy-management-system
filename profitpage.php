...
if (!in_array('total_profits', $_SESSION['permissions'])) {
...
if (isset($_POST['calculate_profit'])) {
    ...
    switch ($timeframe) {
        case '24 hour':
            ...
            break;
        case 'today':
            $date_limit = "DATE(created_at) = CURDATE()";
            break;
        case '1 day':
    ...
    // Combined SQL query for profit calculation from both tables
    if ($date_limit) { // Ensure date_limit is set before executing the query
        $profit_query = "
            SELECT SUM(profit) AS total_profit FROM profits WHERE $date_limit
            UNION ALL
            SELECT SUM(profit) AS total_profit FROM cosmetic_profits WHERE $date_limit
        ";
...
function calculate_profit($conn, $interval) {
    ...
    $query = "
        SELECT SUM(profit) AS total_profit FROM profits WHERE created_at >= NOW() - INTERVAL $interval
        UNION ALL
        SELECT SUM(profit) AS total_profit FROM cosmetic_profits WHERE created_at >= NOW() - INTERVAL $interval
    ";

    $result = mysqli_query($conn, $query);
    return $result ? array_sum(array_column(mysqli_fetch_all($result, MYSQLI_ASSOC), 'total_profit')) : 0;
}

// Calculate daily profits for different timeframes
$today_profit = calculate_profit($conn, '1 DAY');
...
// Uncomment if you want to display yesterday's profit
...
<head>
    ...
    <link rel="stylesheet" href="styles/profitpage.css">
...
<?php include('nav.php'); ?>
<div class="container">
    <h1>Medicine Profit Overview</h1>
